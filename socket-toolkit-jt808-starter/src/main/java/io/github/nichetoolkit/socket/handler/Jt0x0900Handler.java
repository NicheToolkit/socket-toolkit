...
/**
 * <p>Jt0x0900Handler 数据上行透传</p>
 * @author Cyan (snow22314@outlook.com)
 * @version v.1.0
 */
@Slf4j
@SocketPackage(messageId = 0x0900)
public class Jt0x0900Handler implements SocketPackageHandler {
    ...
    public byte[] handle(byte[] phoneBytes, byte[] flowIdBytes, byte[] messageIdBytes, byte[] messageBodyBytes) {
        log.info("[Jt0x0900] 0900 [数据上行透传] data transport");
        threadPoolExecutor.execute(() -> {
            String phone = ByteHexUtils.parseHex(phoneBytes);
            Jt808DataTransport jt808DataTransport = AnalyzeUtils.analyzeDataTransport(messageBodyBytes);
            dataService.dataTransport(phone, jt808DataTransport);
        });
