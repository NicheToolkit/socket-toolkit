...
/**
 * <p>Jt0x0108Handler 终端升级结果通知</p>
 * @author Cyan (snow22314@outlook.com)
 * @version v.1.0
 */
@Slf4j
@SocketPackage(messageId = 0x0108)
public class Jt0x0108Handler implements SocketPackageHandler {
    ...
    public byte[] handle(byte[] phoneBytes, byte[] flowIdBytes, byte[] messageIdBytes, byte[] messageBodyBytes) {
        log.info("[Jt0x0108] 0108 [终端升级结果通知] terminal update result report");
        String phone = ByteHexUtils.parseHex(phoneBytes);
        /** 保存命令到相应的下发指令 */
        threadPoolExecutor.execute(() ->
                dataService.terminalAnswer(phone, -1, "8108", "0108", messageBodyBytes));
